...
from protectmiddlewareapp.models import *
from protectmiddleware.protectmiddleware import ProtectMiddleware
import datetime
...
class SimpleTest(TestCase):
    ...
    def test_correct_key(self):
        ...
        resp = ProtectMiddleware().process_request(request)
        self.assertEqual(resp.status_code, 200)
        self.assertNotEqual(resp._get_content().find(ProtectMiddleware().PROTECTED_ACCESS_GRANTED.split('{')[0]), -1)
    ...
    def test_incorrect_key(self):
        ...
        self.assertNotEqual(resp._get_content().find(ProtectMiddleware().PROTECTED_INCORRECT_KEY), -1)
    ...
    def test_unauthorized_no_session(self):
        ...
        self.assertNotEqual(resp, None)
        self.assertEqual(resp.status_code, 403)
        self.assertNotEqual(resp._get_content().find(ProtectMiddleware().PROTECTED_SITE_NOT_PUBLIC_MSG), -1)
    ...
    def test_authorized_accesspage(self):
        ...
        self.assertNotEqual(resp, None)
        self.assertEqual(resp.status_code, 200)
        self.assertNotEqual(resp._get_content().find(ProtectMiddleware().PROTECTED_ACCESS_GRANTED_ALREADY), -1)
    ...
    def test_admin_unauthorized(self):
        with self.settings(PROTECTED_ADMIN_KEY='99999999998888888888'):
            ...
            request.session.sessionkey='3333333333'
            resp = ProtectMiddleware().process_request(request)
            self.assertNotEqual(resp._get_content().find(ProtectMiddleware().PROTECTED_INCORRECT_ADMIN_KEY), -1)
    ...
    def test_admin_correct_key(self):
        with self.settings(PROTECTED_ADMIN_KEY='99999999998888888888'):
            ...
            request.session.sessionkey='3333333333'
            resp = ProtectMiddleware().process_request(request)
            self.assertNotEqual(resp._get_content().find(ProtectMiddleware().PROTECTED_NEW_ACCESSKEY_CREATED.split('{')[0]), -1)
    ...
    def test_no_session(self):
        ...
        self.assertNotEqual(resp._get_content().find(ProtectMiddleware().PROTECTED_NO_SESSION), -1)
